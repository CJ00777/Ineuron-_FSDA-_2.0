...
select date_trunc (month, Start_date), count(*) as Monthly_subscriptions
from subscriptions
where plan_id =0
group by 1
order by 1
...
from subscriptions as S Inner JOIN Plans as P
...
count(case when plan_id =4 then Customer_id end) as Cust_Churned,
Round(Cust_Churned/Total_Cust_Count *100,1) as Churn_Percentage
from Subscriptions

select P.plan_id, count(distinct customer_id)
...
row_number() over (Partition by Customer_Id Order by S.Plan_Id) as Ranking
from subscriptions as S
Inner Join plans as P
on S.Plan_ID = P.Plan_Id
...
-- Next plan after the initial free trial

select a.customer_id,a.plan_id, a.plan_name
from
(select customer_Id,S.Plan_Id,Plan_Name,
row_number() over (Partition by Customer_Id Order by S.Plan_Id) as Ranking
from subscriptions as S
...
-- or using LEAD (it is faster than ROW_NUMBER)
...
Select * from CJ7_Employee;

-- Row_number
Select *, Row_number () OVER (partition by JOB_ROLE Order By Salary desc) as Salary_Row
from CJ7_Employee;

-- RANK FUNCTION : It skips the actual rank

-- I want top 5 highly paid employees as per salary
Select * from
(Select *, RANK () OVER (partition by JOB_ROLE Order By Salary desc) as Salary_RANK
from CJ7_Employee) as A
where A.Salary_Rank <= 5

-- In this case actual 5th person rank is skip as bcoz rank functions skips the actual rank.
---------------------------------------------------------------------------------------------------------------------------------
-- UDEMY
-- RANK

-- Give me the name of employee who ranked 8th in their respective department.

Select a.first_name, a.department, a.salary, a.RANK_by_dep from
(Select first_Name, department, salary,
rank () over (partition by department order by salary desc) as RANK_by_dep
from employees)a
where RANK_by_dep =8;

-- Give me the top 5 employees from each department
...
Select * from
(Select *, DENSE_RANK () OVER (partition by JOB_ROLE Order By Salary desc) as Salary_RANK
from CJ7_Employee) as A
where A.Salary_Rank <= 5

-- In this case actual 5th person rank is not skip as bcoz dense_rank functions returns the actual rank.
----------------------------------------------------------------------------------------------------------------------------------
-- All three clubbed (ROW, RANK, DENSE_RANK)
...
select sum(salary) as Total_salary from CJ7_employee;
...
-- display total salary as per category-wise with all the records ()every row value 

SELECT * , SUM(SALARY) OVER(PARTITION BY JOB_role) AS TOT_SALARY
FROM CJ7_EMPLOYEE
order by job_role
---------------------------------------------------------------------------------------------------------------------------
-- display the maximum salary per job profile 

select job_role, max(salary) as Max_Salary
group by 1;

-- display the maximum salary per job profile for all the rows 

SELECT *,MAX(SALARY) OVER(PARTITION BY JOB_ROLE) AS MAX_JOB_SAL
FROM CJ7_EMPLOYEE
order by job_role;

-- max/min/total salary as per job profile
...
-- Find out the total salary paid by each department and also region_wise 
-- with their employee names

Select first_name, department, salary, region_id,
sum(salary) Over (Partition by department) total_departmental_salary,
sum(salary) Over (Partition by region_id) total_regional_salary
from employees;

-- Find out the total count of each department and also region_wise 
-- with their employee names

Select first_name, department, 
count(employee_id) Over (Partition by department) total_emp_dep,
region_id,
count(employee_id) Over (Partition by region_id) total_emp_reg
from employees;
...
--  Running total of salaries as per department-wise
...
INSERT INTO CJ7_EMPLOYEE VALUES('101','ANAND JHA','Analyst',50000);
INSERT INTO CJ7_EMPLOYEE VALUES(102,'ALex', 'Data Engineer',60000);
INSERT INTO CJ7_EMPLOYEE VALUES(103,'Ravi', 'Data Scientist',48000);
INSERT INTO CJ7_EMPLOYEE VALUES(104,'Peter', 'Analyst',98000);
INSERT INTO CJ7_EMPLOYEE VALUES(105,'Pulkit', 'Data Scientist',72000);
INSERT INTO CJ7_EMPLOYEE VALUES(106,'Robert','Analyst',100000);
INSERT INTO CJ7_EMPLOYEE VALUES(107,'Rishabh','Data Engineer',67000);
INSERT INTO CJ7_EMPLOYEE VALUES(108,'Subhash','Manager',148000);
INSERT INTO CJ7_EMPLOYEE VALUES(109,'Michaeal','Manager',213000);
INSERT INTO CJ7_EMPLOYEE VALUES(110,'Dhruv','Data Scientist',89000);
INSERT INTO CJ7_EMPLOYEE VALUES(111,'Amit Sharma','Analyst',72000);

INSERT INTO CJ7_EMPLOYEE VALUES(120,'AAMIR','Analyst',50000);
INSERT INTO CJ7_EMPLOYEE VALUES(119,'SAMEER','Analyst',50000);
INSERT INTO CJ7_EMPLOYEE VALUES(112,'ALex', 'Data Engineer',60000);
INSERT INTO CJ7_EMPLOYEE VALUES(117,'ALex', 'Data Engineer',60000);
INSERT INTO CJ7_EMPLOYEE VALUES(117,'ALex', 'Data Engineer',45000);
INSERT INTO CJ7_EMPLOYEE VALUES('101','ANAND JHA','Analyst',40000);
INSERT INTO CJ7_EMPLOYEE VALUES('101','ANAND JHA','Analyst',45000);
INSERT INTO CJ7_EMPLOYEE VALUES('101','ANAND JHA','Analyst',45000);

Delete from CJ7_employee where Job_role = 'Michael'
