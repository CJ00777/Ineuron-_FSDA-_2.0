...
Create or replace procedure select_rec()
returns varchar
language SQL
as
$$
...
BEGIN
select * from bank_details;
message := 'Bank_details added successfully';
return message;
end;
$$
;
...
Create or replace procedure technician_emp()
returns varchar
language SQL
as
$$
...
message varchar;
BEGIN
delete from bank_details where job = 'technician';
message := 'Technician employees deleted successfully';
return message;
end;
$$
;
...
create or replace procedure delete_bc_data (in_JOB varchar)
RETURNS VARCHAR
LANGUAGE SQL
AS
$$
...
MESSAGE VARCHAR;
BEGIN
DELETE FROM BANK_DETAILS WHERE JOB =: in_JOB;
MESSAGE := in_JOB || 'EMPLOYEES DATA DELETED SUCCESSFULLY';
RETURN MESSAGE;
END;
$$;

CALL delete_bc_data ('blue-collar');
