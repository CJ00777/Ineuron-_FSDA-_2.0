...
select * from CJ_departments

create table CJ7_regions (
region_id int,
region varchar(20),
country varchar(20),
...
where department not in (select department from CJ_departments) --  4 records are not present in departments table.

select distinct c.department from
(select e.department
from employees as e left outer join CJ_departments as d
on e.department = d.department
where d.department is null ) c


select distinct d.department
from  CJ_departments as d  left outer join employees as e
on e.department = d.department
where e.department is null 

---------------------------------------------------------------------------------------------------------------------------
select a.employee_name, a.yearly_salary
from (select first_name employee_name,salary yearly_salary from employees where salary > 150000) as a
     (select department employee_name from departments) as b
-------------------------------------------------------------------------------------------------------------------------------------
-- Return all the employees who work in Electronics division
select * from employees
where department in (select department from CJ_departments where division ='Electronics')

-------------------------------------------------------------------------------------------------------------------------------

-- Return the employees who work in Asia or Canada and make over $130000 .

select * from employees
where salary > 130000
and region_id in (select region_id from CJ7_regions where country = 'Asia' or country = 'Canada')

select *
from employees e join CJ7_regions r
on e.region_id = r.region_id
where salary > 130000
and country in ('Asia','Canada')
...
from employees
where region_id in (select region_id from CJ7_regions where country = 'Asia' or country = 'Canada')

----------------------------------------------------------------------------------------------------------------------------

-- ANY, NULL OPERATORS
-- These operators are used with WHERE CLAUSE OR HAVING CLAUSE
-- ANY operator returns true if any of the subquery values meet the condition and ALL operator returns true if all of the subquery values meet the condition.
-- 


select * from employees
where region_id > ALL (select region_id from CJ7_regions where country= 'United States');

-- Above query will return all the values of region_id which is greater then the region_id of United states.

-- Write a query that returns all of those employees that work in the kids division and the dates at which those employees hire is 
-- greater than all of the hire_dates of employees who work in maintenance department.

select * from employees
where department in ( select department from CJ_departments where division ='Kids')
and hire_date > All (select hire_date from employees where department ='Maintenance')
order by department
----------------------------------------------------------------------------------------------------------------------------------
-- Return the highest paid salary which is exactly getting same by other employee
select salary from (
   select salary, count(*)
   from employees
   group by 1
...
-- Return all the unique names in the table with their id

select min(id), name
from dupes
group by name

-- Return the average of salary excluding the MIN & MAX of Salary
...
-- write a SQL statement that returns the names of those students that are taking the courses Physics and US History.
...
select student_name 
from students as S join student_enrollment as SE on S.student_no = SE.student_no
                   join courses as C on SE.course_no = C.Course_no
...
where student_no in  (select student_no from
                     (select student_no, count(course_no) as Count_courses from student_enrollment
                      group by student_no
...
select student_name
from
(select student_name, count(course_no)
from students S join student_enrollment as SE on S.Student_no = SE.Student_no
group by 1
order by 2 desc
limit 1)a
...
(select student_name, max(age) as oldest
from students
group by 1
order by 2 desc
limit 1)
...
select student_name 
from students where age in (select max(age) oldest from students)
...
insert into CJ7_regions values (1, 'Southwest', 'United States');
insert into CJ7_regions values (2, 'Northeast', 'United States');
insert into CJ7_regions values (3, 'Northwest', 'United States');
insert into CJ7_regions values (4, 'Central', 'Asia');
insert into CJ7_regions values (5, 'East Asia', 'Asia');
insert into CJ7_regions values (6, 'Quebec', 'Canada');
insert into CJ7_regions values (7, 'Nova Scotia', 'Canada');
