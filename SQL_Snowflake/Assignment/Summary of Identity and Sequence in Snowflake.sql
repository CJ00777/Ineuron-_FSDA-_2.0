...
create or replace table CJ_COLORS as
    select name
    from (values ('blue'),('red'),('green')) colors (name);

describe table CJ_COLORS;
select * from CJ_COLORS;
    
-- Next we create a new table with the same structure as the existing table and add an idenity column.    
create or replace table Cj_identity_column_example like CJ_COLORS;

alter table Cj_identity_column_example 
add column id int identity(1,3); --default value is 1 and it will increment by 3

insert into Cj_identity_column_example(name)
select name from Cj_colors;
    
select * from CJ_identity_column_example;
...
alter table CJ_IDENTITY_COLUMN_EXAMPLE rename to colors;
...
CREATE OR REPLACE SEQUENCE CJ_SEQ
start = 1
INCREMENT = 5
...
--DONT INSERT RECORDS AS AUTOINCREMENT WORKS ON EMPTY TABLE
...
insert into identity_column_example(c_custkey, c_name, c_address, c_nationkey, c_phone, c_acctbal, c_mktsegment, c_comment)
values (219875, 'Customer#000219874', 'sadhjekj', 9, '19-505-461-2873', 999.99, 'asjdasgdj', 'sdadtyhkjer');

select * from identity_column_example 
where c_custkey = 219875;
order by id asc;

-- if you want to change the default value and increment then create 2nd sequence and use alter table with modify and then
--use seq2 during insert time.
-----------------------------------------------------------------------------------------------------
