...
from  CJ_departments -- 24 rows

select distinct department
from employees -- 27 rows
----------------------------------------------------------------------------------------------------------------------
select distinct e.department, d.department
from employees as e inner join CJ_departments as d
on e.department = d.department  -- 23 rows 
-- (Inner JOin returns the matches records from both the table)

select distinct e.department, d.department
from employees as e left outer join CJ_departments as d
on e.department = d.department
 -- (Left outer join or Left join returns all the record present in left table regardless they are present on other table or not)

select distinct e.department emp_department, d.department 
from employees as e right outer join CJ_departments as d
on e.department = d.department

select distinct e.department emp_department, d.department 
from employees as e Full outer join CJ_departments as d
on e.department = d.department -- returns all the total unique record present in both the table (28 records)
---------------------------------------------------------------------------------------------------------------------------------------
select distinct department from employees
UNION
select distinct department from CJ_departments 
-- Union records eliminates all the duplicate data by combining both the table and retuns the unique one (28 records)
-- Gives the preference to the first table
-- The order, data type and number of columns in select clause in  both the table must be same.


select distinct department from employees
UNION ALL
select distinct department from CJ_departments 
-- UNION ALL doesnt eliminate duplicates, all the records are returned.

select distinct department from employees
EXCEPT
select distinct department from CJ_departments 
-- EXCEPT operator checks all the records from first table and removes it all rows found in other table set.
-- Returns the unmatching records which are found in first table but giving preference to th first table.
